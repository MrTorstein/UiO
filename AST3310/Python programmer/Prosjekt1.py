...
class Energy():
    ...
    def _Sanitytest(self):
        ...
        # Printer total energiproduksjon for hver av kjedene. CNO er kjede fire. #
        for i in range(len(self.eps) - 1):
            print("Total energiproduksjon for reaksjonskjede %i: %2e" %(i + 1, self.eps[i]))

    def _Energiproduksjon(self):
        """ Regner ut total energiproduksjon """

        # Energiproduksjoen for PPI, PPII, PPIII og CNO kjedene respektivt, #
        # og for den totale energiproduksjonen ved et punkt                 #
        self.eps = zeros(5)                             # [J/skg]

        self.eps[0] = (2 * self.Q[0] + self.Q[1]) * self.r_[1]                                                  # PPI
        self.eps[1] = (self.Q[0] + self.Q[2]) * self.r_[2] + self.Q[3] * self.r_[3] + self.Q[4] * self.r_[4]    # PPII
        self.eps[2] = (self.Q[0] + self.Q[2]) * self.r_[2] + self.Q[5] * self.r_[5]                             # PPIII
        self.eps[3] = self.r_[6] * self.Q[6]                                                                    # CNO
        self.eps[4] = self.Q[0] * self.r_[0] \
                    + self.Q[1] * self.r_[1] \
                    + self.Q[2] * self.r_[2] \
                    + self.Q[3] * self.r_[3] \
                    + self.Q[4] * self.r_[4] \
